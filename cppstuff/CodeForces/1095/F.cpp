...
vi arr;
vector<pi> dsu; //parent, rank

int findparent(int k){
    int pt=k;
    while (dsu[pt].f!=pt) pt = dsu[pt].f;
    dsu[k].f = pt;
    return pt;
}
int kruskal(int n){
    ...
    while (n--){
        while (true) {
            int a = edge[pt].s.f, b = edge[pt].s.s;

            if (findparent(a) != findparent(b)) {
                ans += edge[pt].f;
                if (dsu[findparent(a)].s > dsu[findparent(b)].s) {
                    dsu[findparent(a)].s += dsu[findparent(b)].s;
                    dsu[findparent(b)].f = findparent(a);
                } else {
                    dsu[findparent(b)].s += dsu[findparent(a)].s;
                    dsu[findparent(a)].f = findparent(b);
                }
                break;
            }
            pt++;
...
signed main() {
    ...
    for (int i=0;i<n;i++){
        if (i!=minind) {
            edge.pb({arr[i]+minim,{i+1,minind+1}} );
        }
    }
    for (int i=0;i<m;i++){
        int a,b,c;
